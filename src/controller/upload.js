...
module.exports = class extends Base {
  ...
  async bduploadAction() {
    const file = this.ctx.file('image'); // 获取file信息
    const reader = fs.createReadStream(file.path); // 要被拷贝的源文件
    const stream = fs.createWriteStream(path.join(think.ROOT_PATH, 'www/static/upload', file.name)); // 写入数据位置，名字
  ...
  async localToQiniuAction() {
    ...
    const data = await toolUpload(file.name, file.path); // 文件上传
